...
- 配置工作目录
- 配置启动命令
- 配置 transport 类型：`acp` 或 `legacy_cli`
- 记录运行状态
- 停止或重启 agent
...
ACP Adapter 的输出必须归一化为 `.tendrilflow/tasks/{task_id}/events.jsonl` 中的事件，避免 UI、Task Board 或 Coordinator 直接依赖某个 provider 的协议细节。
